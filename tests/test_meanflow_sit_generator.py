...
def test_e15_meanflow_sit_mixed_face_full_resume_config() -> None:
    from safa.training import g_loop

    path = REPO_ROOT / "configs" / "medium_v2" / "experiments" / "e15_meanflow_sit_b_face_mixed_resume_e14_2400ep.yaml"
    assert path.is_file()
    config = yaml.safe_load(path.read_text(encoding="utf-8"))

    assert config["experiment_name"] == "e15_meanflow_sit_b_face_mixed_resume_e14_2400ep"
    assert config["device"] == "cuda:0"
    ...
    assert config["resume_from"] == "artifacts/checkpoints/e15_meanflow_sit_b_face_mixed_resume_e14_2400ep/last.pt"
    assert config["resume_mode"] == "training_state"
    assert config["resume_optimizer_state"] is True
    assert config["out_dir"] == "artifacts/checkpoints/e15_meanflow_sit_b_face_mixed_resume_e14_2400ep"
    ...
    assert quality_eval["output_dir"] == "artifacts/eval/e15_meanflow_sit_b_face_mixed_resume_e14_2400ep/quality"
    assert quality_eval["model"] == "ema"
