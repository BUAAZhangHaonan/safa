...
tmux new-session -d -s train_g "$RUN_ENV $PYTHON_BIN scripts/guarded_run.py --max-ram-fraction $MAX_RAM_FRACTION -- $PYTHON_BIN -m torch.distributed.run --standalone --nproc_per_node=2 -m safa.cli.train_g --config configs/train_g.yaml 2>&1 | tee artifacts/logs/train_g.log"
