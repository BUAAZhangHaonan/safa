#!/usr/bin/env python3
"""Round 3 GPU 3: 2048-sample FID + Sharpness for r3_qv_pu_lora_gpu3."""
...
CKPT = REPO / "artifacts/checkpoints/r3_qv_pu_lora_gpu3/best.pt"
...
OUT_DIR = REPO / "artifacts/r3_eval_r3_qv_pu_lora_gpu3"
...
def main():
    ...
    print(f"[r3_eval_r3_qv_pu_lora_gpu3] running full eval (3969 samples)")
    from safa.evaluation.runner import run_eval_from_config
    run_eval_from_config(config)
    print(f"[r3_eval_r3_qv_pu_lora_gpu3] eval done, computing FID")
    ...
    print(f"[r3_eval_r3_qv_pu_lora_gpu3] FID(2048) = {{fid_value:.4f}}")
    sharp_values = []
    ...
    print(f"[r3_eval_r3_qv_pu_lora_gpu3] DONE, FID={{fid_value:.4f}} sharp={{sharp_mean:.2f}}")
