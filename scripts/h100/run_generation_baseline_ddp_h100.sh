...
Options:
  ...
  --ablation-only        Allow this internal ablation queue to dry-run or run.
...
if [[ "${ABLATION_ONLY}" -ne 1 ]]; then
  echo "Refusing to run H100 DDP generation baseline queue by default: E17/E18/E20/E21/E22/E23 are internal ablation experiments, not paper main-table mature baselines." >&2
...
if [[ "${DRY_RUN}" -eq 1 ]]; then
  print_plan "DRY RUN: internal ablation H100 DDP queue; pass --run with --ablation-only to start training"
  exit 0
fi

print_plan "RUN: starting internal ablation H100 DDP generation baseline queue"
run_queue
