...
tmux new-session -d -s train_e0 "$RUN_ENV $PYTHON_BIN scripts/guarded_run.py --max-ram-fraction $MAX_RAM_FRACTION -- $PYTHON_BIN -m safa.cli.train_e0 --config configs/train_e0.yaml 2>&1 | tee artifacts/logs/train_e0.log"
