...
CONFIG="configs/medium_v2/experiments/r14_inpaint_resume_gpu12_epoch100.yaml"
ARTIFACT_ROOT="artifacts/r14_inpaint_resume_gpu12_epoch100/v1"
CHECKPOINT_ROOT="checkpoints/r14_inpaint_resume_gpu12_epoch100"
...
VERIFY=(
  "$PYTHON_BIN" scripts/validate_r14_inpaint_resume_gpu12_epoch100.py
  --mode artifact
)
...
print_pipeline() {
  printf '%s\n' \
    "Locked R14 long resume training (CUDA_VISIBLE_DEVICES=$GPU_LIST, nproc=$NPROC):" \
    "  source step: 2432" \
    "  source completed stage2 epochs: 19" \
    "  additional stage2 epochs: 81" \
    "  additional optimizer steps: 10368" \
    "  required final step: 12800" \
    "  checkpoint steps: 2560,5120,7680,10240,12800" \
...
case "$MODE" in
  ...
  launch)
    ...
    TMUX_PAYLOAD=(
      ...
      bash scripts/run_r14_inpaint_resume_gpu12_epoch100.sh --inside-tmux
    )
    ...
    printf 'Started %s on physical GPU%s. Log: %s\n' "$SESSION" "$GPU_LIST" "$LOG"
    ;;
  *)
