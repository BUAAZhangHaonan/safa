...
tmux new-session -d -s cache_e0_val "$RUN_ENV $PYTHON_BIN scripts/guarded_run.py --max-ram-fraction $MAX_RAM_FRACTION -- $PYTHON_BIN -m safa.cli.cache_e0 --config configs/cache_e0_val.yaml 2>&1 | tee artifacts/logs/cache_e0_val.log"
